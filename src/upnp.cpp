...
namespace sgns::upnp
{
    std::vector<std::string> GetLocalIPv4CandidatesFromOS() {
        ...
        if (GetAdaptersAddresses(AF_INET, GAA_FLAG_INCLUDE_GATEWAYS, nullptr, adapterAddresses, &bufferSize) == NO_ERROR) {
    ...
    bool UPNP::GetIGD() {
        ...
        for (const auto& candidateIp : bindCandidates) {
            ...
            for (auto target : search_targets) {
                ...
                m_logger->trace("Sending SSDP M-SEARCH for '{}' from {}", target, local_endpoint.address().to_string());

                socket_->async_send_to(boost::asio::buffer(*request), _multicast,
                    [self, local_endpoint, timer, request](const boost::system::error_code& error, size_t bytes_sent) {
                        if (!error) {
                            ...
                            self->socket_->async_receive_from(head_begin, *remote_endpoint,
                                [self, headerbuff, local_endpoint, timer](const boost::system::error_code& receive_error, size_t bytes_received) {
                                    if (!receive_error) {
                                        timer->cancel();
                ...
                timer->async_wait([self](const boost::system::error_code& timer_error) {
                    if (!timer_error) {
                        self->socket_->cancel();
    ...
    bool UPNP::SendSOAPRequest(std::string soaprq, std::string& result)
    {
        ...
        try {
            ...
            m_logger->warn("Unable to bind SOAP socket to preferred local IP ({}). Falling back to OS routing.", e.what());
        }
        //Make Soap Buffer
